...
option::ArgStatus IsEtendueD(const option::Option& option, bool msg)
{
	...
	tm time_one;
	tm time_two;
	char * strp_return1  = strptime(first_date.c_str(), "%d/%m/%Y", &time_one);
	char * strp_return2  = strptime(second_date.c_str(), "%d/%m/%Y", &time_two);

	if (strp_return1 == nullptr || strp_return2 == nullptr)
	{
		cout << time_one.tm_year   << " " << time_one.tm_mon   << " " << time_one.tm_mday   << " " << time_two.tm_year   << " " << time_two.tm_mon   << " " << time_two.tm_mday ;
		cerr << "L'étendue datée doit être de la forme \"JJ/MM/AAAA-JJ/MM/AAAA\"" << endl;
		return option::ARG_ILLEGAL;
	}
...
int main(int argc, char* argv[])
{
	...
	measurementsReader mesReader(string("./data/measurements.csv"), ';', localisationVector, rayonEffectif, t_value, flag_d, flag_t, d_value);
	// Mesure mes;
	...
	if(options[IMPACT]) {
		...
		Agence agence(0);
		agence.accederDonnees(mesReader);
